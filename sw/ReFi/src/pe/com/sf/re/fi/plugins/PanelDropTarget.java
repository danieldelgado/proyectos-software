...
public class PanelDropTarget implements DropTargetListener {
	...
	public void dragEnter(DropTargetDragEvent dtde) {
		DnDUtils3.debugPrintln("dragEnter, drop action = " + DnDUtils3.showActions(dtde.getDropAction()));
	...
	public void dragExit(DropTargetEvent dte) {
		DnDUtils3.debugPrintln("DropTarget dragExit");
	}

	public void dragOver(DropTargetDragEvent dtde) {
		DnDUtils3.debugPrintln("DropTarget dragOver, drop action = " + DnDUtils3.showActions(dtde.getDropAction()));
	...
	public void dropActionChanged(DropTargetDragEvent dtde) {
		DnDUtils3.debugPrintln("DropTarget dropActionChanged, drop action = " + DnDUtils3.showActions(dtde.getDropAction()));
	...
	public void drop(DropTargetDropEvent dtde) {
		DnDUtils3.debugPrintln("DropTarget drop, drop action = " + DnDUtils3.showActions(dtde.getDropAction()));
		...
		if ((dtde.getDropAction() & DnDConstants.ACTION_COPY_OR_MOVE) != 0) {
			...
			try {
				...
				DnDUtils3.debugPrintln("Drop completed, success: " + result);
			} catch (Exception e) {
				DnDUtils3.debugPrintln("Exception while handling drop " + e);
				dtde.dropComplete(false);
			}
		} else {
			DnDUtils3.debugPrintln("Drop target rejected drop");
			dtde.rejectDrop();
		}
	}
	...
	protected boolean acceptOrRejectDrag(DropTargetDragEvent dtde) {
		...
		if (!acceptableType || (sourceActions & DnDConstants.ACTION_COPY_OR_MOVE) == 0) {
			DnDUtils3.debugPrintln("Drop target rejecting drag");
			dtde.rejectDrag();
		} else if ((dropAction & DnDConstants.ACTION_COPY_OR_MOVE) == 0) {
			// Not offering copy or move - suggest a copy
			DnDUtils3.debugPrintln("Drop target offering COPY");
			dtde.acceptDrag(DnDConstants.ACTION_COPY);
			acceptedDrag = true;
		} else {
			// Offering an acceptable operation: accept
			DnDUtils3.debugPrintln("Drop target accepting drag");
	...
	protected void checkTransferType(DropTargetDragEvent dtde) {
		...
		DnDUtils3.debugPrintln("File type acceptable - " + acceptableType);
	}
	...
	protected boolean dropComponent(Transferable transferable) throws IOException, UnsupportedFlavorException {
		...
		if (o instanceof Component) {
			DnDUtils3.debugPrintln("Dragged component class is " + o.getClass().getName());
