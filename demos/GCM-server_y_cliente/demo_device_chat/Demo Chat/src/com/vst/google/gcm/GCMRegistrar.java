...
//	private static final String PROPERTY_ON_SERVER = "onServer";
	...
	public static void checkDevice(Context context) {
		...
		/* 75 */if (version < 8) {
			/* 76 */throw new UnsupportedOperationException(new StringBuilder()
					.append("Device must be at least API Level 8 (instead of ")
	...
	public static void checkManifest(Context context) {
		...
		/* 116 */String permissionName = new StringBuilder()
				.append(packageName).append(".permission.C2D_MESSAGE")
				.toString();
		try {
			/* 119 */packageManager.getPermissionInfo(permissionName, 4096);
		} catch (PackageManager.NameNotFoundException e) {
			/* 122 */throw new IllegalStateException(new StringBuilder()
					.append("Application does not define permission ")
					.append(permissionName).toString());
		}

		PackageInfo receiversInfo;
		try {
			/* 128 */receiversInfo = packageManager.getPackageInfo(packageName,
					2);
		} catch (PackageManager.NameNotFoundException e) {
			/* 131 */throw new IllegalStateException(new StringBuilder()
					.append("Could not get receivers for package ")
					.append(packageName).toString());
		}

		/* 134 */ActivityInfo[] receivers = receiversInfo.receivers;
		/* 135 */if ((receivers == null) || (receivers.length == 0)) {
			/* 136 */throw new IllegalStateException(new StringBuilder()
					.append("No receiver for package ").append(packageName)
					.toString());
		}
		...
		/* 139 */if (Log.isLoggable("GCMRegistrar", 2)) {
			/* 140 */Log.v(
					"GCMRegistrar",
					new StringBuilder().append("number of receivers for ")
							.append(packageName).append(": ")
							.append(receivers.length).toString());
		}

		/* 143 */Set allowedReceivers = new HashSet();
		/* 144 */for (ActivityInfo receiver : receivers) {
			/* 145 */if ("com.google.android.c2dm.permission.SEND"
	...
	private static void checkReceiver(Context context,
		...
		/* 166 */List<ResolveInfo> receivers = pm.queryBroadcastReceivers(
				intent, 32);

		/* 168 */if (receivers.isEmpty()) {
			/* 169 */throw new IllegalStateException(new StringBuilder()
					.append("No receivers for action ").append(action)
					.toString());
		}

		/* 172 */if (Log.isLoggable("GCMRegistrar", 2)) {
			/* 173 */Log.v("GCMRegistrar", new StringBuilder().append("Found ")
					.append(receivers.size()).append(" receivers for action ")
		...
		/* 177 */for (ResolveInfo receiver : receivers) {
			...
			/* 179 */if (!allowedReceivers.contains(name))
				/* 180 */throw new IllegalStateException(new StringBuilder()
						.append("Receiver ").append(name)
						.append(" is not set with permission ")
						.append("com.google.android.c2dm.permission.SEND")
						.toString());
		}
	}
	...
	public static void register(Context context, String senderIds) {
		/* 202 */setRetryBroadcastReceiver(context);
		/* 203 */resetBackoff(context);
		/* 204 */internalRegister(context, new String[] { senderIds });
	}
	...
	static void internalRegister(Context context, String[] senderIds) {
		...
		/* 216 */Log.v(
				"GCMRegistrar",
				new StringBuilder().append("Registering app ")
						.append(context.getPackageName())
						.append(" of senders ").append(senders).toString());

		/* 218 */Intent intent = new Intent(
				"com.google.android.c2dm.intent.REGISTER");
		/* 219 */intent.setPackage("com.google.android.gsf");
		/* 220 */intent.putExtra("app",
	...
	static void internalUnregister(Context context) {
		/* 255 */Log.v(
				"GCMRegistrar",
				new StringBuilder().append("Unregistering app ")
						.append(context.getPackageName()).toString());
		/* 256 */Intent intent = new Intent(
				"com.google.android.c2dm.intent.UNREGISTER");
		/* 257 */intent.setPackage("com.google.android.gsf");
		/* 258 */intent.putExtra("app",
				PendingIntent.getBroadcast(context, 0, new Intent(), 0));
	...
	private static synchronized void setRetryBroadcastReceiver(Context context) {
		/* 267 */if (sRetryReceiver == null) {
			...
			/* 274 */String permission = new StringBuilder().append(category)
					.append(".permission.C2D_MESSAGE").toString();
			/* 275 */Log.v("GCMRegistrar", "Registering receiver");
			/* 276 */context.registerReceiver(sRetryReceiver, filter,
					permission, null);
		}
	}
	...
	public static String getRegistrationId(Context context) {
		...
		/* 295 */if ((oldVersion != -2147483648) && (oldVersion != newVersion)) {
			/* 296 */Log.v(
					"GCMRegistrar",
					new StringBuilder().append("App version changed from ")
							.append(oldVersion).append(" to ")
							.append(newVersion)
							.append("; resetting registration id").toString());
	...
	static String setRegistrationId(Context context, String regId) {
		...
		/* 332 */Log.v("GCMRegistrar",
				new StringBuilder().append("Saving regId on app version ")
	...
	public static void setRegisteredOnServer(Context context, boolean flag) {
		...
		/* 345 */Log.v(
				"GCMRegistrar",
				new StringBuilder()
						.append("Setting registered on server status as: ")
	...
	public static boolean isRegisteredOnServer(Context context) {
		...
		/* 357 */Log.v(
				"GCMRegistrar",
				new StringBuilder().append("Is registered on server: ")
						.append(isRegistered).toString());
	...
	private static int getAppVersion(Context context) {
		try {
			...
			/* 371 */throw new RuntimeException(new StringBuilder()
					.append("Coult not get package name: ").append(e)
					.toString());
		}
	}
	...
	static void resetBackoff(Context context) {
		/* 383 */Log.d(
				"GCMRegistrar",
				new StringBuilder().append("resetting backoff for ")
						.append(context.getPackageName()).toString());
	...
	private static SharedPreferences getGCMPreferences(Context context) {
		/* 415 */return context.getSharedPreferences("com.google.android.gcm",
				0);
	}
