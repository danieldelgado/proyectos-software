...
public final class GCMRegistrar {
	...
	public static void checkDevice(Context context) {
		...
		/* 75 */if (version < 8) {
			/* 76 */throw new UnsupportedOperationException(new StringBuilder().append("Device must be at least API Level 8 (instead of ")
	...
	public static void checkManifest(Context context) {
		...
		try {
			...
			/* 122 */throw new IllegalStateException(new StringBuilder().append("Application does not define permission ").append(permissionName)
					.toString());
		}
		...
		try {
			...
			/* 131 */throw new IllegalStateException(new StringBuilder().append("Could not get receivers for package ").append(packageName)
					.toString());
		}
		...
		/* 139 */if (Log.isLoggable("GCMRegistrar", 2)) {
			/* 140 */Log.v("GCMRegistrar",
					new StringBuilder().append("number of receivers for ").append(packageName).append(": ").append(receivers.length).toString());
		}
	...
	private static void checkReceiver(Context context, Set<String> allowedReceivers, String action) {
		...
		/* 172 */if (Log.isLoggable("GCMRegistrar", 2)) {
			/* 173 */Log.v("GCMRegistrar",
					new StringBuilder().append("Found ").append(receivers.size()).append(" receivers for action ").append(action).toString());
		}

		/* 177 */for (ResolveInfo receiver : receivers) {
			/* 178 */String name = receiver.activityInfo.name;
			/* 179 */if (!allowedReceivers.contains(name))
				/* 180 */throw new IllegalStateException(new StringBuilder().append("Receiver ").append(name).append(" is not set with permission ")
						.append("com.google.android.c2dm.permission.SEND").toString());
		}
	}
	...
	static void internalRegister(Context context, String[] senderIds) {
		...
		/* 216 */Log.v("GCMRegistrar",
				new StringBuilder().append("Registering app ").append(context.getPackageName()).append(" of senders ").append(senders).toString());
	...
	public static String getRegistrationId(Context context) {
		...
		if ((oldVersion != -2147483648) && (oldVersion != newVersion)) {
			Log.v("GCMRegistrar", new StringBuilder().append("App version changed from ").append(oldVersion).append(" to ").append(newVersion)
	...
	private static int getAppVersion(Context context) {
		try {
			...
			/* 371 */throw new RuntimeException(new StringBuilder().append("Coult not get package name: ").append(e).toString());
		}
	}
